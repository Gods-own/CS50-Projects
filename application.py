...
def index():
    ...
    if numOfRows > 0:
        ...
        # Get all the stocks the user has so: Query the database for the stock symbol and company name to get the stocks that have
        # a sum of shares greater than 0 in order to display on the page
        ...
        # Get the current price of each stocks and the sum of shares of each stock the user has as long as their sum is greater than 0
...
def history():
    ...
    # Query the database for the activity of the user
    rows = db.execute("SELECT * FROM activity WHERE user_id = ?", session["user_id"])

    # Get the number of rows tha above query returned
    numOfRows = len(rows)

    # Check if the number of rows of the user is greater than 0
    if numOfRows > 0:

         # Query the database for the activity of the user which include the stock symbol, shares, price and date
        ...
        return render_template("history.html", numOfRows=numOfRows)
...
def register():
    ...
    if request.method == "POST":
        ...
        uncomment. This is gotten from geeksforgeeks.org"""
...
def sell():
    ...
    if request.method == "POST":
     ...
     # user reached route via GET
        ...
        # Query the database for all the stock symbol in order to pass it into the select options as long as the sum of the shares is
        # greater than 0
